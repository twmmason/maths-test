...
const CHIEF_SYSTEM = `You are the Chief Engineer at Mission Control, answering questions from Commander Artie, a 10-year-old rocket engineer in England.
...
export async function askChiefEngineer(question: string, task?: GeneratedTask): Promise<string> {
  const prompt = `${task ? `Artie is currently working on this engineering task (do NOT solve it, its answer is ${task.answer} — never mention or hint at it):\n"${task.briefing}"\n\n` : ""}Artie asks: "${question}"

Answer the concept question in a friendly, age-appropriate way without solving the task.`;
  const out = await generateText(prompt, CHIEF_SYSTEM, 6000);
